...
static void luaL_register2(lua_State* L, const char* parent, const char* libname, const luaL_Reg* reg)
{
    ...
    const gchar *message, lua_State* L)
{
    ...
    luaL_where(L, 1);
    if(lua_rawlen(L, -1) == 0) luaL_where(L, 1);
    if(lua_rawlen(L, -1) == 0) lua_pushstring(L, "? ");
...
{   
    ...
    #ifdef IDEBUG
    fprintf(stderr, "Calling index_closure for key %s!\n", p);
    #endif
...
static void _wrap_glib_ret(lua_State* L)
{
    ...
    luaL_register2(L, "gobject", "Object", object);
    ...
    lua_rawset(L, LUA_REGISTRYINDEX);
    luaL_register2(L, "gobject", "Type", type);
