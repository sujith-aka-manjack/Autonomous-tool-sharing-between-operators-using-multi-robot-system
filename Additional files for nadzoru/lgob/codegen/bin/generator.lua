...
package.path = package.path .. ';codegen/src/?.lua'
...
for cname, func in pairs(defFunctions) do
    func.name = cname
    ti(sFuncs, func)
end
...
for pos, func in ipairs(sFuncs) do
    if not propertyAvailable(func.accessorSince, v1, v2) then
        ...
        if defOverrides[func.name] then
            ...
            -- Return value
            if func.ret then
                local ret = defClasses[func.ret] and 'special' or func.ret
            ...
            if func.args then
                for apos, arg in ipairs(func.args) do
                    ...
                    -- heuristic to find unhandled 'struct* none'
                    if t.isunhandled and arg:match("^const .-* none$") then
                        t = Types['struct* none']
                    end
        ...
        if #tables['unhandled'] > 0 then
            local reasons = table.concat(tables['unhandled'], '\n    ')
            tables['unhandled'] = {}
            log(sf('Function %s not handled.\nReasons:\n    %s\n', func.name, reasons))
        else
