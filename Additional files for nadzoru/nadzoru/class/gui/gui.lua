...
Gui = letk.Class( function( self )
    ...
    self:append_menu_item('file', "_Close All Tab", "Close All Tabs", 'gtk-delete', self.remove_all_tabs, self )
...
---Close all tabs.
--@param self Gui
function Gui:remove_all_tabs( )
    local n = self.note:get_n_pages()
    while self.note:get_n_pages() ~= 0 do
        self:remove_tab(0)
    end
    self.window:show_all()
end
