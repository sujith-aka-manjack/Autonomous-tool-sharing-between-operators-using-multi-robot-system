...
function Controller:build()
    ...
    self.gui:append_menu_item('automata', "_Remove All", "Remove all Automata"      , 'gtk-delete', self.remove_all_automaton    , self)
...
---Removes all automaton from the list of editable automata.
--Creates a selector showing all automata. Removes the selected automata from the elements list. Closes the all removed automata tabs.
--TODO CHECK
--@param self
--@see Selector:add_multipler
--@see Selector:run
--@see Gui:remove_all_tabs
function Controller:remove_all_automaton()
    self.elements = letk.List.new()
    self.gui:remove_all_tabs()
end
