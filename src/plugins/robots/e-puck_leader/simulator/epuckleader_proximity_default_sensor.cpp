/**
 * @file <argos3/plugins/simulator/sensors/epuckleader_proximity_default_sensor.cpp>
 *
...
#include "epuckleader_proximity_default_sensor.h"
...
namespace argos {
   ...
   CEPuckLeaderProximityDefaultSensor::CEPuckLeaderProximityDefaultSensor() :
   ...
   void CEPuckLeaderProximityDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
   ...
   void CEPuckLeaderProximityDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_EPuckLeaderProximitySensor::Init(t_tree);
         /* Show rays? */
   ...
   void CEPuckLeaderProximityDefaultSensor::Update()
   {
   ...
   void CEPuckLeaderProximityDefaultSensor::Reset()
   {
   ...
   Real CEPuckLeaderProximityDefaultSensor::CalculateReading(Real f_distance)
   {
      //return Exp(-f_distance);

      // from the e-puck_leader model by Lorenzo Garattoni and Gianpiero Francesca
      Real value = 0.0f;
   ...
   REGISTER_SENSOR(CEPuckLeaderProximityDefaultSensor,
                   "epuckleader_proximity", "default",
                   "Danesh Tarapore [daneshtarapore@gmail.com]",
                   "1.0",
                   "The E-Puck proximity sensor.",

                   "This sensor accesses the epuckleader proximity sensor. For a complete description\n"
                   "of its usage, refer to the ci_epuckleader_proximity_sensor.h interface. For the XML\n"
