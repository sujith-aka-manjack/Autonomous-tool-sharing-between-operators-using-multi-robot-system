/**
 * @file <epuckpos/simulator/dynamics3d_epuckpos_model.cpp>
 *
 * @author Michael Allwright - <allsey87@gmail.com>
 */

#include "dynamics3d_epuckpos_model.h"
...
namespace argos {
   ...
   CDynamics3DEPuckPosModel::CDynamics3DEPuckPosModel(CDynamics3DEngine& c_engine,
                                                CEPuckPosEntity& c_epuckpos) :
      /* technically, the CDynamics3DMultiBodyObjectModel should be initialized with 7 children
         links, however, setting it to 7 makes the epuckpos less stable for reasons. */
      CDynamics3DMultiBodyObjectModel(c_engine, c_epuckpos, 3, false),
      m_cWheeledEntity(c_epuckpos.GetWheeledEntity()) {
      ...
      const SAnchor& sOriginAnchor = c_epuckpos.GetEmbodiedEntity().GetOriginAnchor();
      ...
      SAnchor* psBodyAnchor = &c_epuckpos.GetEmbodiedEntity().AddAnchor("body", {0.0, 0.0, 0.00125});
      /* create the bodies */
   ...
   void CDynamics3DEPuckPosModel::Reset() {
   ...
   void CDynamics3DEPuckPosModel::CalculateBoundingBox() {
   ...
   void CDynamics3DEPuckPosModel::UpdateEntityStatus() {
   ...
   void CDynamics3DEPuckPosModel::UpdateFromEntityStatus() {
      /* run the base class's implementation of this method */
   ...
   void CDynamics3DEPuckPosModel::AddToWorld(btMultiBodyDynamicsWorld& c_world) {
      /* run the base class's implementation of this method */
   ...
   void CDynamics3DEPuckPosModel::RemoveFromWorld(btMultiBodyDynamicsWorld& c_world) {
   ...
   REGISTER_STANDARD_DYNAMICS3D_OPERATIONS_ON_ENTITY(CEPuckPosEntity, CDynamics3DEPuckPosModel);

   /****************************************/
   /****************************************/

   const btVector3    CDynamics3DEPuckPosModel::m_cBodyHalfExtents(0.0362, 0.0236, 0.0362);
   const btScalar     CDynamics3DEPuckPosModel::m_fBodyMass(0.242);
   const btTransform  CDynamics3DEPuckPosModel::m_cBodyOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0,0.00125,0.0));
   const btTransform  CDynamics3DEPuckPosModel::m_cBodyGeometricOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0, -0.0236, 0.0));
   const btVector3    CDynamics3DEPuckPosModel::m_cWheelHalfExtents(0.02125,0.0015,0.02125);
   const btScalar     CDynamics3DEPuckPosModel::m_fWheelMass(0.006);
   const btTransform  CDynamics3DEPuckPosModel::m_cWheelGeometricOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0,-0.0015,0.0));
   const btTransform  CDynamics3DEPuckPosModel::m_cLeftWheelOffset(btQuaternion(btVector3(-1,0,0), SIMD_HALF_PI), btVector3(0.0, 0.02125, -0.0255));
   const btTransform  CDynamics3DEPuckPosModel::m_cRightWheelOffset(btQuaternion(btVector3(1,0,0), SIMD_HALF_PI), btVector3(0.0, 0.02125, 0.0255));
   const btVector3    CDynamics3DEPuckPosModel::m_cBodyToRightWheelJointOffset(0.0, -0.0036, 0.0255);
   const btVector3    CDynamics3DEPuckPosModel::m_cRightWheelToBodyJointOffset(0.0, 0.0015, 0.0);
   const btQuaternion CDynamics3DEPuckPosModel::m_cBodyToRightWheelJointRotation(btVector3(-1,0,0), SIMD_HALF_PI);
   const btVector3    CDynamics3DEPuckPosModel::m_cBodyToLeftWheelJointOffset(0.0, -0.0036, -0.0255);
   const btVector3    CDynamics3DEPuckPosModel::m_cLeftWheelToBodyJointOffset(0.0, 0.0015, -0.0);
   const btQuaternion CDynamics3DEPuckPosModel::m_cBodyToLeftWheelJointRotation(btVector3(1,0,0), SIMD_HALF_PI);
   /* TODO calibrate these values */
   const btScalar     CDynamics3DEPuckPosModel::m_fWheelMotorMaxImpulse(0.15);
   const btScalar     CDynamics3DEPuckPosModel::m_fWheelFriction(5.0);
