/**
 * @file <argos3/plugins/simulator/sensors/epuckpos_proximity_default_sensor.cpp>
 *
...
#include "epuckpos_proximity_default_sensor.h"
...
namespace argos {
   ...
   CEPuckPosProximityDefaultSensor::CEPuckPosProximityDefaultSensor() :
   ...
   void CEPuckPosProximityDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
   ...
   void CEPuckPosProximityDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_EPuckPosProximitySensor::Init(t_tree);
         /* Show rays? */
   ...
   void CEPuckPosProximityDefaultSensor::Update()
   {
   ...
   void CEPuckPosProximityDefaultSensor::Reset()
   {
   ...
   Real CEPuckPosProximityDefaultSensor::CalculateReading(Real f_distance)
   {
      //return Exp(-f_distance);

      // from the e-puck_pos model by Lorenzo Garattoni and Gianpiero Francesca
      Real value = 0.0f;
   ...
   REGISTER_SENSOR(CEPuckPosProximityDefaultSensor,
                   "epuckpos_proximity", "default",
                   "Danesh Tarapore [daneshtarapore@gmail.com]",
                   "1.0",
                   "The E-Puck proximity sensor.",

                   "This sensor accesses the epuckpos proximity sensor. For a complete description\n"
                   "of its usage, refer to the ci_epuckpos_proximity_sensor.h interface. For the XML\n"
